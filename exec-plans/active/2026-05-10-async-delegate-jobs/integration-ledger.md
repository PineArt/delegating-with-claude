...
- Orchestrator / Removed high-level synchronous `run` while preserving bridge diagnostics / `scripts/claude_delegate.py`, `README.md`, `SKILL.md`, `tests/test_claude_delegate.py` / Orchestrator / 35 passing tests, py_compile, diff check, harness validator / Accepted as scoped follow-up / Commit and push, then record publish evidence.
