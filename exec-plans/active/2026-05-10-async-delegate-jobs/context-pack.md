...
- The agreed design is an async-only job model with explicit `start/status/wait/stop/resume`.
...
- The old no-subcommand invocation and the high-level synchronous `run` command should be removed rather than quietly preserved.
